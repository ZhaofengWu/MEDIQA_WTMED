...
from mt_dnn.tree import Tree, NON_BREAKING_SPACE, NON_BREAKING_SPACE_ESCAPE

def _treeify(string, skip_long_parses=False):
    string = string.replace(NON_BREAKING_SPACE, NON_BREAKING_SPACE_ESCAPE)
    if '(' not in string:
        string = f'({string})'
    string = re.sub(r'\(', '(N ', string)  # because binary parses don't have labels
    string = re.sub(r' \(', '(', string)  # compress repr
    string = re.sub(r'\( ', '(', string)
    string = re.sub(r' \)', ')', string)
    string = re.sub(r'\) ', ')', string)
    bin_parse = Tree.from_string(string)
    bin_parse.assert_binary()
    return bin_parse if not skip_long_parses or len(bin_parse.str_repr) < 1500 else None
...
def load_mednli(file, label_dict, header=False):
    rows = []
    cnt = 0
    with open(file, encoding="utf8") as f:
        for line in f:
            ...
            sample = {'uid': blocks[0], 'premise': blocks[1], 'hypothesis': blocks[2],
                      'parse_id_a': _treeify(blocks[3], skip_long_parses=False).char_indices(),
                      'parse_id_b': _treeify(blocks[4], skip_long_parses=False).char_indices(),
                      'label': lab}
            rows.append(sample)
            cnt += 1
    return rows
