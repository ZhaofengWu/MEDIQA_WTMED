...
class MTDNNModel(object):
    def __init__(self, opt, state_dict=None, num_train_step=-1,
        ...
        self.network = SANBertNetwork(opt,
                                      use_parse=use_parse, embedding_matrix=embedding_matrix,
                                      token2idx=token2idx, stx_parse_dim=stx_parse_dim, unked_words=unked_words,
                                      use_generic_features=use_generic_features, num_generic_features=num_generic_features,
                                      use_domain_features=use_domain_features, num_domain_features=num_domain_features, feature_dim=feature_dim)
    ...
    def update(self, batch_meta, batch_data,
        ...
        logits = self.mnetwork(*inputs,
                               bin_parse_as=self._value_for('parse_ids_a', batch_data, batch_meta),
                               bin_parse_bs=self._value_for('parse_ids_b', batch_data, batch_meta),
                               parse_as_mask=self._value_for('parse_masks_a', batch_data, batch_meta),
                               parse_bs_mask=self._value_for('parse_masks_b', batch_data, batch_meta),
                               generic_features=self._value_for('generic_features', batch_data, batch_meta),
                               domain_features=self._value_for('domain_features', batch_data, batch_meta))
